...
export class PancakeSwapService {
  ...
  private readonly ROUTER_ADDRESS =
  ...
  private async getToken(tokenAddress: string): Promise<Token> {
    ...
    try {
      ...
      this.logger.log(
        `Token loaded: ${symbol} (${tokenAddress}) with ${decimals} decimals`,
      );
  ...
  async getSwapQuote(params: SwapQuoteDto): Promise<any> {
    try {
      ...
      if (
        !trade &&
        fromToken.address.toLowerCase() !== this.BUSD_ADDRESS.toLowerCase()
      ) {
        try {
      ...
      if (
        !trade &&
        fromToken.address.toLowerCase() !== this.USDT_ADDRESS.toLowerCase()
      ) {
        try {
  ...
  async createUnsignedSwapTransaction(params: SwapQuoteDto): Promise<any[]> {
    try {
      ...
      if (isNativeIn) {
        ...
        const totalRequired =
          BigInt(currencyAmount.quotient.toString()) + estimatedGasCost;
        ...
        const data = routerInterface.encodeFunctionData(
          ...
          [minimumAmountOut.quotient.toString(), swapPath, recipient, deadline],
        );
      ...
      // ═══════════════════════════════════════════════════════════
        ...
        const swapPath: string[] = [...path]; // Copy path
        ...
        if (isNativeOut) {
          ...
          if (lastAddress !== wbnbAddress) {
            this.logger.warn(
              `Fixing path: last token ${lastAddress.slice(0, 6)} → WBNB ${wbnbAddress.slice(0, 6)}`,
            );
          ...
          this.logger.log(
            `Token → Token | Final Path: ${swapPath.join(' → ')}`,
          );
        }

...
export class EvmTxExhaustedReconcilerService {
  ...
  async poll(): Promise<void> {
    if (this.isRunning) {
      this.logger.warn('evm tx exhausted reconciliation previous run still in progress, skipping.');
      return;
    }
    ...
    try {
      ...
      const pending = await this.exhaustedOrderRepository.findPendingSince(swapProvider.EVMTX, since);
      if (!pending.length) return;

      this.logger.log(`evm tx reconciliation processing ${pending.length} exhausted order(s)`);
      for (const order of pending) {
        try {
          await this.reconcileOrder(order);
        } catch (err) {
          this.logger.error(`[${order.txHash}] evm tx reconciliation failed`, err);
        }
      }
    } finally {
  ...
  private async reconcileOrder(order: LeanExhaustedOrder): Promise<void> {
    const orderResult = await this.repo.findByTxHash(order.txHash);
    if (!orderResult.ok || !orderResult.data) {
      this.logger.warn(`[${order.txHash}] Could not load swap order, cannot reconcile`);
      return;
    }

    const { fromChain } = orderResult.data;
    const newStatus = await this.txReceiptStatusService.getStatus(fromChain, order.txHash);
    if (!newStatus) {
      this.logger.log(`[${order.txHash}] Still unresolved, keeping for next reconciliation run`);
      return;
    }

    const updatedOrder = await this.repo.updateOrderStatus(order.txHash, newStatus);

...
export class FusionPlusExhaustedReconcilerService {
  private readonly logger = new Logger(
    FusionPlusExhaustedReconcilerService.name,
  );
  ...
  async poll(): Promise<void> {
    if (this.isRunning) {
      this.logger.warn(
        'fusion+ exhausted reconciliation previous run still in progress, skipping.',
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `fusion+ reconciliation processing ${pending.length} exhausted order(s)`,
      );
      for (const order of pending) {
        try {
          const resumed = await this.inchService.resumeSecretRevealPolling(
            order.txHash,
          );
          if (!resumed) {
            this.logger.warn(
          ...
          this.logger.error(
            `[${order.txHash}] fusion+ reconciliation failed`,
            err,
          );
        }
      }

...
export class BanxaWebhookService {
  ...
  async handleWebHook(dto: any): Promise<void> {
    try {
      ...
      const currentDevice = await this.deviceService.findOne(
        dto.external_id as unknown as mongoose.Schema.Types.ObjectId,
      );
      if (currentDevice === null) {
        this.logger.error(`=== device not found ====`);
        return;
      }
      const notificationObj = this.getOrderNotification(dto);
      await this.processTxNotification(notificationObj, currentDevice.fcmToken);
    } catch (error) {
      this.logger.error('banxa notification error:', error);
    }
  }

  private getOrderNotification(order: Order): OrderNotification {
    ...
    switch (order.status) {
      ...
      case 'cancelled':
        return {
          ...
          message:
            'Your order has been cancelled due to compliance or risk checks.',
        };
      ...
      case 'extraVerification':
        return {
          ...
          message:
            'Your order requires additional verification. Support will contact you shortly.',
        };
  ...
  private async processTxNotification(

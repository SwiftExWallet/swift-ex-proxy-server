...
import { FusionSDK, OrderStatus as SingleChainOrderStatus } from '@1inch/fusion-sdk';
...
export class FustionNativeService {
    ...
    public getProvider(chainId: number): ethers.JsonRpcProvider {
        if (!this.providers.has(chainId)) {
            ...
            if (!chainSuffix) {
                throw new BadRequestException(`Chain ID ${chainId} is not supported by your lookup mapping.`);
            }
            const envKey = `PROVIDER_RPC_${chainSuffix}_1`;
            const rpcUrl = this.configService.get<string>(envKey);

            if (!rpcUrl) {
                throw new BadRequestException(`RPC configuration missing for ${envKey}. Please add it to your .env file.`);
            }
        ...
        if (!provider) {
            throw new NotFoundException(`Failed to initialize provider context for chain: ${chainId}`);
        }
    ...
    async createSwapOrder(dto: FusionPlusSwapQuoteDto) {
        try {
            const { amount, srcChain, dstChain, srcTokenAddress, dstTokenAddress, walletAddress } = dto;
            ...
            if (isSingleChainSwap) {
                this.logger.log(`Executing Single-Chain Fusion flow on chain ${srcChainId}`);
                ...
                const orderInfo = await fusionSdk.submitNativeOrder(preparedOrder.order, makerAddress, preparedOrder.quoteId);
                this.logger.debug(`Single-Chain Order Submitted! Hash: ${orderInfo.orderHash}`);
            ...
            this.logger.log(`Executing Cross-Chain Fusion+ flow from ${srcChainId} to ${dstChainId}`);
            ...
            const message = error.response?.data?.description || error.message || 'unable to build swap';
    ...
    async confirmSwapOrder(dto: ConfirmSwapOrderDto) {
        try {
            ...
            if (!secretState) {
                throw new NotFoundException('Order context was not found or has expired.');
            }

            const chainId = ChainId[srcChain];
            const provider = this.getProvider(chainId);

            this.logger.log(`Awaiting 3 confirmations for tx: ${txHash} on chain: ${chainId}`);

            provider.waitForTransaction(txHash, 3)
                .then(() => {
                    this.logger.log(`Transaction ${txHash} confirmed. Spawning settlement routines.`);
            ...
            return { success: true, typeTx:"fusion",message: 'Fulfillment loop initiated on backend.' };
        } catch (error: any) {
            this.logger.error(error);
            const message = error.response?.data?.description || error.message || 'unable to confirm swap';
            throw new BadRequestException(message);
        }
    }
        ...
        try {
            while (true) {
                ...
                if (!secretState.isCrossChain) {
                    ...
                    this.logger.log(`[Loop-SingleChain] Order ${hash} current status: ${orderStatus.status}`);
                    ...
                    if (
                        ...
                        if (orderStatus.status === SingleChainOrderStatus.Expired) targetDbStatus = SwapOrderStatus.EXPIRED;
                        if (orderStatus.status === SingleChainOrderStatus.Cancelled) targetDbStatus = SwapOrderStatus.REFUNDED;
                        ...
                        this.logger.log(`[Loop-SingleChain] Sequence finalized with status: ${orderStatus.status}. Cleaning Redis.`, "DB status:", orderStatusUpdate);
                ...
                const secretsToShare = await this.crossChainSdk.getReadyToAcceptSecretFills(hash);
                ...
                if (secretsToShare.fills.length) {
                    for (const { idx } of secretsToShare.fills) {
                        ...
                        stateUpdated = true;
                        this.logger.log(`[Loop] Shared secret for index ${idx} on order ${hash}`);
                    }
                }
                ...
                this.logger.log(`[Loop-CrossChain] Order ${hash} current status: ${status}`);
                ...
                if (
                    ...
                    this.logger.log(`[Loop-CrossChain] Execution sequence finalized with status: ${status}. Cleaning Redis.`, "DB status:", orderStatusUpdate);
            ...
            this.logger.error(`[Loop Error] Failure observed executing actions on ${hash}:`, error);
        }
    }
}
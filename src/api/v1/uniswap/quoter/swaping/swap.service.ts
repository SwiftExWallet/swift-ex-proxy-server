...
import { ChainEnum, ChainId } from 'src/api/v1/common/enums/chain.enum';
...
export class SwapService {
    ...
    private readonly SINGLE_QUOTER_CONTRACT_ADDRESS = process.env.SINGLE_QUOTER_CONTRACT_ADDRESS as string;
    ...
    private getProvider(chain: ChainEnum): ethers.JsonRpcProvider {
    ...
    private async tryMultiHopQuote(
        ...
        try {
            const quoterContract = new Contract(
                CHAIN_CONFIGS[ChainEnum[chain]].uniswapV3QuoterV2,
                this.QUOTER_V2_ABI,
                this.getProvider(ChainEnum[chain]),
            );
            ...
            this.logger.warn('-4-Multi-hop quote error:', error.message);
            return null;
        }
    }
    ...
    async getQuote(swapQuoteDto: SwapQuoteDto): Promise<SwapQuote> {
        try {
            ...
            const tokenInAddress = isNativeIn ? CHAIN_CONFIGS[ChainEnum[swapQuoteDto.chainId]] : swapQuoteDto.tokenIn.address;
            const tokenOutAddress = isNativeOut ? CHAIN_CONFIGS[ChainEnum[swapQuoteDto.chainId]] : swapQuoteDto.tokenOut.address;

            if (
                tokenInAddress.toLowerCase() === CHAIN_CONFIGS[ChainEnum[swapQuoteDto.chainId]] &&
                isNativeOut
            ) {
                return {
            ...
            const tokenIn = new Token(
                CHAIN_CONFIGS[ChainEnum[swapQuoteDto.chainId]].chainId,
                tokenInAddress,
            ...
            const quoterContract = new Contract(
                CHAIN_CONFIGS[ChainEnum[swapQuoteDto.chainId]].uniswapV3QuoterV2,
                QUOTER_V2_ABI,
                this.getProvider(ChainEnum[swapQuoteDto.chainId]),
            );
            ...
            for (const feeTier of FEE_TIERS) {
                try {
                    ...
                    amountOut = await quoterContract.quoteExactOutputSingle.staticCall({
                    ...
                    if (feeTier === FEE_TIERS[FEE_TIERS.length - 1]) {
                        ...
                        const multiHopResult = await this.tryMultiHopQuote(
                            ...
                            tokenIn.decimals,
                            tokenOut.decimals,
                            ChainEnum[ChainId[swapQuoteDto.chainId]]
                        );
            ...
            const formattedAmountOut = formatUnits(amountOut[0], tokenOut.decimals);
            const pricePerToken =
            ...
            const feeData = await this.getProvider(ChainEnum[swapQuoteDto.chainId]).getFeeData();
    ...
    async buildSwapTx(
        ...
        try {
            ...
            const [nonce, feeData, balance] = await Promise.all([
                this.getProvider(ChainEnum[swapQuoteDto.chainId]).getTransactionCount(fromAddress, "pending"),
                this.getProvider(ChainEnum[swapQuoteDto.chainId]).getFeeData(),
                this.getProvider(ChainEnum[swapQuoteDto.chainId]).getBalance(fromAddress)
            ]);
            ...
            if (quote.isWethUnwrap) {
                ...
                const wethContract = new Contract(this.WETH_ADDRESS, ERC20_ABI, this.getProvider(ChainEnum[swapQuoteDto.chainId]));
                ...
                const rawTx: TransactionRequest = {
                    ...
                    data: withdrawData,
                    value: 0n,
                    chainId: CHAIN_CONFIGS[ChainEnum[swapQuoteDto.chainId]].chainId,
                    nonce,
                    type: 2,
                    maxFeePerGas,
                ...
                try {
                    const estimatedGas = await this.getProvider(ChainEnum[swapQuoteDto.chainId]).estimateGas(rawTx);
            ...
            if (isNativeIn) {
                ...
                try {
                    ...
                    this.logger.warn('-1-Gas estimation failed, using default:', gasError.message);
                }
                ...
                const tokenContract = new Contract(tokenIn.address, ERC20_ABI, this.getProvider(ChainEnum[swapQuoteDto.chainId]));
                ...
                if (currentAllowance < amountInWei) {
                    ...
                    try {
                        const approveGas = await this.getProvider(ChainEnum[swapQuoteDto.chainId]).estimateGas(approveTx);
                        approveTx.gasLimit = (approveGas * 110n) / 100n;
                    } catch (gasError) {
                        approveTx.gasLimit = 60000n;
                        this.logger.warn('-2-Approve gas estimation failed, using default:', gasError.message);
                    }
                ...
                try {
                    ...
                    rawTx.gasLimit = quote.isMultiHop ? 300000n : 200000n;
                    console.warn('-3-Swap gas estimation failed, using default:', gasError.message);
                }

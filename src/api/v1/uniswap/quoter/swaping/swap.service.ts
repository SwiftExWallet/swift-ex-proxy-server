...
export class SwapService {
  ...
  private resolveSupportedChain(chainId: any): SupportedChain {
    return ChainEnum[chainId] as unknown as SupportedChain;
  }

  private getProvider(chain: SupportedChain): ethers.JsonRpcProvider {
    return this.providerService.getProvider(chain as unknown as ChainEnum);
  }
  ...
  private router(chain: SupportedChain): string {
    return this.cfg(chain).swapRouter;
  }

  private wrapped(chain: SupportedChain): string {
    return this.cfg(chain).wrappedNative;
  }

  private isNative(value: string): boolean {
    const list = [
  ...
  private async gasPricing(chain: SupportedChain) {
    ...
    return {
      maxFeePerGas: fee.maxFeePerGas ?? gasPrice,
  ...
  async buildSwapTx(
    ...
    try {
      const quote = await this.getQuote(dto);
      ...
      const router = this.router(chain);

      const wrapped = this.wrapped(chain);

      const iface = new Interface(ETH_PREPARE_ABI);
      ...
      const quoteOutWei = parseUnits(quote.outputAmount, tokenOut.decimals);

      const minOut = (quoteOutWei * BigInt(10000 - slippageBps)) / 10000n;
      ...
      if (isNativeIn) {
        ...
        tx.gasLimit = await this.estimateGasWithBuffer(tx, chain, 180000n);

        const totalNeed =
          amountInWei + this.toBigInt(tx.gasLimit) * gas.maxFeePerGas;

        if (nativeBalance < totalNeed) {
          throw new BadRequestException(
            `Insufficient ${config.nativeSymbol}. Have ${formatUnits(
              nativeBalance,
              18,
            )}, Need ~${formatUnits(totalNeed, 18)}`,
          );
        }
      ...
      if (tokenBalance < amountInWei) {
        throw new BadRequestException(`Insufficient ${tokenIn.symbol}`);
      }
      ...
      if (allowance < amountInWei) {
        ...
        approveTx.gasLimit = await this.estimateGasWithBuffer(
          approveTx,
          chain,
          60000n,
        );
      ...
      if (isNativeOut) {
        const singleSwap = iface.encodeFunctionData('exactInputSingle', [
          {
            tokenIn: tokenIn.address,
            tokenOut: wrapped,
            fee: Number(quote.fee),
        ...
        const unwrap = iface.encodeFunctionData('unwrapWETH9', [
          minOut,
          wallet,
        ]);

        swapData = iface.encodeFunctionData('multicall', [
          [singleSwap, unwrap],
        ]);
      } else {
      ...
      swapTx.gasLimit = await this.estimateGasWithBuffer(
        swapTx,
        chain,
        220000n,
      );
      ...
      const totalGas =
        txs.reduce((a, b) => a + this.toBigInt(b.gasLimit ?? 0), 0n) +
        this.toBigInt(swapTx.gasLimit);

      const gasCost = totalGas * gas.maxFeePerGas;

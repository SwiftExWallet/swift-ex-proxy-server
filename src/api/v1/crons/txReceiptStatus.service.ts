...
function mapAlchemyStatus(receipt: { status?: number | null } | null): SwapOrderStatus | null {
    if (!receipt) {
        return null;
    }
...
export class TxReceiptStatusService {
    ...
    constructor() {
        ...
        for (const [chainKey, networkKey] of Object.entries(alchemyNetworkByChain)) {
    ...
    async getStatus(fromChain: string, txHash: string): Promise<SwapOrderStatus | null> {
        ...
        this.logger.debug(`Both blockscout and alchemy have no result yet for txHash ${txHash} chain ${fromChain}`);
        return null;
    }
    ...
    private async fetchFromAlchemy(chainKey: string, txHash: string): Promise<SwapOrderStatus | null> {
        const alchemy = this.getAlchemyClient(chainKey);
        if (!alchemy) {
            this.logger.warn(`No Alchemy client configured for chain ${chainKey}, cannot fallback for txHash ${txHash}`);
            return null;
        }

...
export class NearIntentPollerService implements OnModuleInit {
  ...
  async startPolling(
    ...
    if (this.activePolls.has(orderId)) {
      this.logger.warn(
        `[${orderId}] NEARINTENT poller already active, skipping duplicate start`,
      );
      return;
    }
    ...
    if (memo) {
      ...
      await this.redisService.setKey(
        this.redisKey(orderId),
        JSON.stringify(state),
        REDIS_TTL_SECONDS,
      );
      this.logger.log(
        `[${orderId}] Saved memo to redis (ttl=${REDIS_TTL_SECONDS}s)`,
      );
    }
  ...
  private async recoverPendingOrders(): Promise<void> {
    ...
    for (const order of result.data) {
      ...
      if (!raw) {
        this.logger.warn(
          `[${orderId}] No redis state found, skipping NEARINTENT recovery`,
        );
        continue;
      }
  ...
  private async runPollingCycle(
    ...
    try {
      for (let attempt = 1; attempt <= MAX_POLL_ATTEMPTS; attempt++) {
        this.logger.log(
          `[${orderId}] Poll attempt ${attempt}/${MAX_POLL_ATTEMPTS}`,
        );
        ...
        try {
          ...
          if (errorRetryCount >= MAX_ERROR_RETRIES) {
            this.logger.error(
              `[${orderId}] Max error retries exceeded, stopping poll`,
            );
        ...
        this.logger.log(
          `[${orderId}] Current execution status: ${status.status}`,
        );

        if (
          status.status === GetExecutionStatusResponse.status.PROCESSING &&
          !this.sourceRefreshTriggered.has(orderId)
        ) {
      ...
      this.logger.warn(
        `[${orderId}] Max polling attempts (${MAX_POLL_ATTEMPTS}) exceeded without reaching a terminal state`,
      );
      await this.exhaustPolling(orderId, depositAddress, memo);
      return {
        success: false,
  ...
  private async refreshSourcePortfolio(orderId: string): Promise<void> {
    ...
    if (!result.ok || !result.data) {
      this.logger.warn(
        `[${orderId}] Could not load order for source portfolio refresh`,
      );
      return;
    }
  ...
  private async saveExhaustedForReconciliation(
    ...
    try {
      ...
      this.logger.log(
        `[${orderId}] Saved to ExhaustedOrders for reconciliation`,
      );
    } catch (err) {
      this.logger.error(
  ...
  private async updateOrderStatusAndNotify(
    ...
    const orderStatusUpdate = await this.swapOrderService.updateOrderById(
      orderId,
      orderStatus,
    );
